...
class UserSimilarity(Base):
    ...
    __table_args__ = (UniqueConstraint("user_id", "neighbor_id", name="unique_user_neighbor_pair"),)
    ...
    similarity_score: Mapped[float] = mapped_column(
        Float, index=True
    )  # -1 to 1 (Pearson) or 0 to 1 (cosine)
    overlap_count: Mapped[int] = mapped_column(Integer)  # Number of books both users rated
    adjusted_similarity: Mapped[float] = mapped_column(
        Float, index=True
    )  # After significance weighting
