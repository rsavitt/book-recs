...
def upgrade() -> None:
    ...
    if not column_exists("books", "is_why_choose"):
        op.add_column(
            "books",
            sa.Column("is_why_choose", sa.Boolean(), nullable=False, server_default="false"),
        )
    if not column_exists("books", "why_choose_confidence"):
        op.add_column(
            "books",
            sa.Column("why_choose_confidence", sa.Float(), nullable=False, server_default="0.0"),
        )
    ...
    if not column_exists("users", "exclude_why_choose"):
        op.add_column(
            "users",
            sa.Column("exclude_why_choose", sa.Boolean(), nullable=False, server_default="true"),
        )
